...
const fetchData = async () => {
  return withRetry(() => supabase.from('appointments').select(), {
    maxRetries: 3,
  });
};
```
...
```typescript
const data = await withRetry(() => supabase.from('test').select(), {
  maxRetries: 2,
});
...
const saveData = async input => {
  ...
  // 3. Save with retry
  await withRetry(() => supabase.from('data').insert({ value: clean }), {
    maxRetries: 3,
  });
};
```
