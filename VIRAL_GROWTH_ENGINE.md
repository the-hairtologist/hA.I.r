...
```mermaid
graph TB
    A[Stylist Signs Up] --> B[Gets Referral Code]
    B --> C[Shares Code with 3+ Stylists]
    C --> D[New Stylists Sign Up]
    D --> E[Referrer Gets Rewards]
    E --> F[More Motivation to Share]
    F --> C

    A --> G[Books Clients via hA.I.r]
    G --> H[Clients See Booking Page]
    H --> I[Powered by hA.I.r Branding]
    I --> J[Clients Ask Their Stylists]
    J --> K[More Stylist Signups]
    K --> D

    G --> L[Milestones Trigger]
    L --> M[Client Shares Timeline]
    M --> N[Social Exposure]
    N --> J
```
