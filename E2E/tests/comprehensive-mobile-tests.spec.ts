...
test.describe('Mobile QA - Admin Role', () => {
  ...
  test('2. Mobile Navigation - Admin', async ({ page }) => {
    ...
    const menuButton = page
      .locator('button[aria-label*="menu"], button:has-text("Menu")')
      .first();
  ...
  test('4. Mobile Touch Gestures - Admin', async ({ page }) => {
    ...
    const scrollableElement = page
      .locator('[data-scroll], .overflow-auto, .overflow-scroll')
      .first();
    ...
    const hasSafeArea = await page.evaluate(() => {
      ...
      return (
        style
          ...
          .includes('env(safe-area-inset-top)') ||
  ...
  test('9. Mobile Offline Handling - Admin', async ({ page, context }) => {
    ...
    const hasOfflineIndicator = await page
      .locator('text=offline, text=connection, [data-offline]')
      .first()
      .isVisible()
      .catch(() => false);
...
test.describe('Mobile QA - Stylist Role', () => {
  test.beforeEach(async ({ page }) => {
    await loginMobile(
      page,
      TEST_USERS.stylist.email,
      TEST_USERS.stylist.password
    );
  });
...
test.describe('Mobile QA - Client Role', () => {
  ...
  test('2. Mobile Touch Targets - Client', async ({ page }) => {
    ...
    const cards = await page
      .locator('[data-testid="stylist-card"], .card, button')
      .all();
