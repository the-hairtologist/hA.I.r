...
test.describe('Client Requests', () => {
  ...
  test('should validate field lengths', async ({ page }) => {
    ...
    await expect(page.getByText(/title must be less than 100 characters/i)).toBeVisible();
  });
  ...
  test('should create post successfully', async ({ page }) => {
    ...
    await page.getByLabel(/description/i).fill('Looking for an experienced stylist for balayage');
  ...
  test('should prevent double submission', async ({ page }) => {
    ...
    page.on('request', request => {
      if (request.url().includes('client_hair_posts') && request.method() === 'POST') {
        insertCount++;
      }
    });
