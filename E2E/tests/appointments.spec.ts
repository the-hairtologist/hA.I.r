...
test.describe('Appointments Management', () => {
  ...
  test('should update appointment status with confirmation', async ({ page }) => {
    const firstAppointment = page.locator('[data-testid="appointment-card"]').first();
    await firstAppointment.click();
    ...
    await expect(page.getByRole('dialog', { name: /confirm appointment/i })).toBeVisible();
  ...
  test('should prevent double-clicking status update', async ({ page }) => {
    ...
    page.on('request', request => {
      if (request.url().includes('appointments') && request.method() === 'PATCH') {
        updateCount++;
      }
    });
