...
test.describe('Resource Loading', () => {
  ...
  test('should have acceptable bundle size', async ({ page }) => {
    ...
    const totalTransferred = requests.reduce((sum, req) => sum + (req.transferSize || 0), 0);
...
test.describe('Caching', () => {
  test('should cache static assets', async ({ page }) => {
    ...
    expect(cachedResourcesAfterReload).toBeGreaterThanOrEqual(cachedResources.length);
  });

  test('should use service worker for offline capability', async ({ page, context }) => {
    await page.goto('/');
