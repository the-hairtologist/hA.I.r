...
test.describe('Critical User Journeys', () => {
  ...
  test.describe('Client Appointment Booking Flow', () => {
    test('client can browse stylists', async ({ page }) => {
      ...
      const hasStylists = await page
        .locator('[data-testid*=stylist], .stylist-card')
        .count();

      if (hasStylists > 0) {
        await expect(
          page.locator('[data-testid*=stylist], .stylist-card').first()
        ).toBeVisible();
      }
    });
  ...
  test.describe('AI Formula Generation', () => {
    test('can access AI assistant', async ({ page }) => {
      ...
      const aiLink = page
        .locator('text=AI Assistant, [href*=ai], [aria-label*=AI]')
        .first();
  ...
  test.describe('Authentication Flow', () => {
    test('can navigate to login page', async ({ page }) => {
      ...
      const loginButton = page
        .locator('text=Sign In, text=Login, [href*=login]')
        .first();
    ...
    test('login form has required fields', async ({ page }) => {
      ...
      await expect(
        page.locator('[type=password], [name=password]')
      ).toBeVisible();
    });
    ...
    test('can navigate to signup page', async ({ page }) => {
      ...
      const signupButton = page
        .locator('text=Sign Up, text=Register, [href*=signup]')
        .first();
