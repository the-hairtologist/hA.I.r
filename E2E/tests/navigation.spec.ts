...
test.describe('Navigation Flow Tests', () => {
  ...
  test('should navigate to all main pages from dashboard', async ({ page }) => {
    ...
    const navigationLinks = [
      { selector: 'a[href="/dashboard"]', url: '/dashboard', name: 'Dashboard' },
      { selector: 'a[href="/appointments"]', url: '/appointments', name: 'Appointments' },
      ...
      { selector: 'a[href="/portfolio"]', url: '/portfolio', name: 'Portfolio' },
  ...
  test('should complete new user onboarding flow', async ({ page }) => {
    ...
    const welcomeElement = await page.locator('text=/Welcome|Get Started/i').first();
  ...
  test('should handle booking flow end-to-end', async ({ page }) => {
    ...
    await expect(page.locator('text=/Appointment created|Success/i')).toBeVisible({ timeout: 10000 });
  });

  test('should not have dead-end pages', async ({ page }) => {
    const pages = ['/dashboard', '/appointments', '/clients', '/formulas', '/services'];
    
    for (const pagePath of pages) {
      await page.goto(pagePath);
      
      // Verify back navigation exists (sidebar or back button)
      const navigationExists = await page.locator('nav, button:has-text("Back")').count();
  ...
  test('should preserve state during navigation', async ({ page }) => {
    ...
    const warningOrSaved = await page.locator('text=/unsaved|auto-saved|draft/i').count();
  ...
  test('should handle rapid navigation without breaking', async ({ page }) => {
    const routes = ['/dashboard', '/appointments', '/clients', '/formulas', '/services'];
...
test.describe('Modal & Dialog Navigation', () => {
  ...
  test('should trap focus within modals', async ({ page }) => {
    ...
    const focusableElements = await page.locator('dialog button, dialog input, dialog select').count();
...
test.describe('Error State Navigation', () => {
  test('should handle 404 pages gracefully', async ({ page }) => {
    ...
    const isRedirected = page.url().includes('/dashboard') || page.url().includes('/auth');
  ...
  test('should provide way to recover from errors', async ({ page }) => {
    ...
    const safeNavigation = await page.locator('a[href="/dashboard"], button:has-text("Go Home")').count();
