...
test.describe('Public Stylist Discovery', () => {
  ...
  test('should search stylists by name/specialty', async ({ page }) => {
    ...
    const hasEmptyState = await page.getByText(/no stylists found/i).isVisible();
    
    expect(hasCards || hasEmptyState).toBe(true);
  });

  test('should have working "Discover Stylists Online" button', async ({ page }) => {
    await page.goto('/stylists');
    
    // Find the discover button
    const discoverBtn = page.getByRole('button', { name: /Discover Stylists Online/i });
...
test.describe('Integration Suggestions', () => {
  test('should appear on dashboard when criteria met', async ({ page }) => {
    ...
    const suggestionCard = page.locator('text=/Suggested|Connect Your Calendar|Enable SMS/i').first();
  ...
  test('integration suggestion dismiss should persist', async ({ page, context }) => {
    await page.goto('/dashboard');
    await page.waitForTimeout(2000);
...
test.describe('Mobile Responsiveness - Discovery', () => {
  test('stylist cards should be readable on mobile', async ({ page }) => {
    ...
    if (await card.isVisible()) {
      ...
      const fontSize = await card.locator('h2, h3').first().evaluate(
  ...
  test('touch targets should be 44x44px minimum on mobile', async ({ page }) => {
...
test.describe('Cross-Device Scaling', () => {
  ...
  devices.forEach(({ name, width, height }) => {
    test(`should render properly on ${name}`, async ({ page }) => {
      ...
      console.log(`${name}: Layout OK, No horizontal overflow: ${!hasHorizontalScroll}`);
    });
  });
});

test.describe('Integration Features Verification', () => {
  test('search-stylists edge function should be callable', async ({ request }) => {
    ...
    const response = await request.post(
      'https://iyotklwiwyljospfqnoy.supabase.co/functions/v1/search-stylists',
      {
        headers: {
          ...
          'apikey': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Iml5b3RrbHdpd3lsam9zcGZxbm95Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTkzNTU0NDAsImV4cCI6MjA3NDkzMTQ0MH0.X1bkOZPuNuGeUHYfiN1p8_z8jtRWnfi2T9WlzGOb_jA',
        },
        data: {
          location: 'New York',
          specialty: 'balayage',
    ...
    console.log(`Found ${data.stylists?.length || 0} stylists via web discovery`);
  });

  test('integration marketplace should list all integrations', async ({ page }) => {
    await page.goto('/integrations');
    await page.waitForTimeout(2000);
  ...
  test('recommended integrations should be highlighted', async ({ page }) => {
    ...
    if (hasRecommended) {
      const cards = page.locator('text=/Zapier|Google Calendar|QuickBooks|Instagram/i');
...
test.describe('AI Integration Suggestions', () => {
  test('integration suggestions component should render without errors', async ({ page }) => {
    await page.goto('/dashboard');
...
test.describe('Comprehensive Discovery Flow', () => {
  test('complete user journey: discover → view → book attempt', async ({ page }) => {
