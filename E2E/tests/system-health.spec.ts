...
test.describe('System Health Monitoring', () => {
  ...
  test('should handle network disconnection gracefully', async ({ page, context }) => {
    ...
    const hasOfflineIndicator = await page.locator('text=/offline|connection|network/i').count();
    ...
    // Should recover
    const isRecovered = page.url().includes('/appointments') || page.url().includes('/dashboard');
  ...
  test('should handle rapid button clicks without errors', async ({ page }) => {
    ...
    const newAppointmentButton = page.locator('button:has-text("New Appointment")');
  ...
  test('should recover from API errors', async ({ page }) => {
    ...
    const hasErrorState = await page.locator('text=/error|try again|no appointments/i').count();
