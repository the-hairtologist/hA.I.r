...
test.describe('Background Removal - Mobile', () => {
  ...
  test('should show background removal button with proper touch target', async ({ page }) => {
    await page.goto('/portfolio');
    
    // Find the Sparkles button (background removal trigger)
    const bgRemovalButton = page.locator('button[title="Remove background with AI"]').first();
  ...
  test('should display image previews properly on mobile', async ({ page }) => {
    ...
    if (await bgRemovalButton.isVisible()) {
      ...
      const previewContainer = page.locator('.grid.grid-cols-1.md\\:grid-cols-2').first();
  ...
  test('should show progress indicator during processing', async ({ page }) => {
    ...
    if (await bgRemovalButton.isVisible()) {
      ...
      const processButton = page.locator('button:has-text("Remove Background")');
...
test.describe('Zapier Integration - Mobile', () => {
  ...
  test('should display use cases grid on mobile', async ({ page }) => {
    ...
    const useCaseCards = page.locator('.grid').filter({ hasText: 'Google Calendar' });
...
test.describe('Accessibility - New Features Mobile', () => {
  test('background removal dialog should be keyboard accessible', async ({ page }) => {
    await page.setViewportSize({ width: 390, height: 844 });
    await page.goto('/portfolio');
  ...
  test('Zapier form should have proper labels', async ({ page }) => {
    ...
    const webhookInput = page.locator('input[type="url"]').first();
    const labelFor = await page.locator('label[for="webhook-url"]').textContent();
