...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log(
      `📋 Found ${completedAppointments?.length || 0} appointments for follow-up`
    );
    ...
    console.log(
      `📋 Found ${noShowAppointments?.length || 0} no-shows for re-booking reminder`
    );
    ...
    for (const appointment of noShowAppointments || []) {
      ...
      totalSent++;
      console.log(
        `✅ Sent re-booking reminder for appointment ${appointment.id}`
      );
    }
    ...
    const { data: upcomingBirthdays } = await supabase
      ...
      .lt(
        'birthday',
        new Date(sevenDaysFromNow.getTime() + 24 * 60 * 60 * 1000)
          .toISOString()
          .split('T')[0]
      );

    console.log(
      `🎂 Found ${upcomingBirthdays?.length || 0} upcoming birthdays`
    );

    for (const client of upcomingBirthdays || []) {
      const userInfo = Array.isArray(client.user)
        ? client.user[0]
        : client.user;
      const stylistInfo = Array.isArray(client.stylist)
        ? client.stylist[0]
        : client.stylist;
      const stylistUserInfo = stylistInfo
        ? Array.isArray(stylistInfo.user)
          ? stylistInfo.user[0]
          : stylistInfo.user
        : null;
      ...
      const stylistName =
        stylistUserInfo?.full_name || stylistInfo?.business_name;

      if (!clientEmail || !stylistName) continue;

      const birthdayDate = new Date(client.birthday).toLocaleDateString(
        'en-US',
        {
          month: 'long',
          day: 'numeric',
    ...
    console.log(
      `📅 Found ${clientsNeedingRebook?.length || 0} clients due for rebooking`
    );
    ...
    for (const [, appointment] of clientRebookMap) {
      ...
      if (prefs && !prefs.rebooking_reminders_enabled) {
        console.log(
          `⏭️ Skipping rebook reminder - user opted out: ${clientEmail}`
        );
        continue;
      }

      const lastApptDate = new Date(
