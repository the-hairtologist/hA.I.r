...
serve(async req => {
  ...
  try {
    ...
    console.log(
      `[Calendar Sync] ${action} event for appointment:`,
      appointment_id
    );
    ...
    const { data: tokens, error: tokenError } = await supabaseClient.rpc(
      'get_calendar_token',
    ...
    if (action === 'create' || action === 'update') {
      ...
      const endTime = new Date(
        startTime.getTime() + (appointment.duration_minutes || 90) * 60000
      );
      ...
      const eventData = {
        ...
        description:
          appointment.notes ||
          `Appointment with ${appointment.client.full_name}`,
        start: {
