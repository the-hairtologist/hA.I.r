...
serve(async req => {
  ...
  try {
    ...
    const { appointmentId, notificationType, customMessage }: SMSRequest =
      await req.json();

    console.log(
      `📱 Sending ${notificationType} SMS for appointment ${appointmentId}`
    );
    ...
    const rateLimitHeaders = getRateLimitHeaders(
      remaining,
      resetAt,
      RATE_LIMITS.SMS_NOTIFICATION.maxRequests
    );
