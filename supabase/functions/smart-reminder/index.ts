...
serve(async req => {
  ...
  try {
    ...
    console.log(
      `Found ${appointments?.length || 0} appointments for reminders`
    );
    ...
    for (const apt of appointments || []) {
      try {
        ...
        if (apt.client?.email) {
          ...
          if (
            !emailPrefs ||
            emailPrefs.appointment_reminders_enabled !== false
          ) {
            ...
            console.log(
              `⏭️ Email skipped - user opted out: ${apt.client.email}`
            );
          }
        }
        ...
        if (!emailSent && !smsSent) {
          console.warn(
            `⚠️ No reminders sent for appointment ${apt.id} - both channels opted out or unavailable`
          );
          continue;
        }
