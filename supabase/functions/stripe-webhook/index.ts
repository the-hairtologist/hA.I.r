  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      const appointmentData = JSON.parse(
        session.metadata?.appointment_data || '{}'
      );
      const isDeposit = session.metadata?.is_deposit === 'true';
      const remainingBalance = parseFloat(
        session.metadata?.remaining_balance || '0'
      );
      ...
      if (paymentError) {
        console.error(
          '[STRIPE-WEBHOOK] Error creating payment record:',
          paymentError
        );
        throw paymentError;
      }
      ...
      try {
        ...
        console.error(
          '[STRIPE-WEBHOOK] Failed to trigger Zapier webhook:',
          zapierError
        );
      }
      ...
      try {
        ...
        console.error(
          '[STRIPE-WEBHOOK] Failed to send confirmation SMS:',
          smsError
        );
      }
