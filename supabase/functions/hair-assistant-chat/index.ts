...
serve(async req => {
  ...
  try {
    ...
    if (stylistContext) {
      contextInfo += `\n\n🎨 STYLIST CONTEXT:\n`;
      if (stylistContext.business_name)
        contextInfo += `- Business: ${stylistContext.business_name}\n`;
      if (stylistContext.color_line)
        contextInfo += `- Preferred Color Line: ${stylistContext.color_line}\n`;
      if (stylistContext.specialty)
        contextInfo += `- Specialty: ${stylistContext.specialty}\n`;
      if (stylistContext.years_experience)
        contextInfo += `- Experience: ${stylistContext.years_experience} years\n`;
    }

    if (clientContext) {
      contextInfo += `\n\n👤 CLIENT CONTEXT (${clientContext.full_name}):\n`;
      if (clientContext.hair_type)
        contextInfo += `- Hair Type: ${clientContext.hair_type}\n`;
      if (clientContext.hair_goals)
        contextInfo += `- Hair Goals: ${clientContext.hair_goals}\n`;
      if (clientContext.allergies)
        contextInfo += `- ⚠️ ALLERGIES: ${clientContext.allergies}\n`;
      if (clientContext.sensitivity_notes)
        contextInfo += `- Sensitivities: ${clientContext.sensitivity_notes}\n`;
      if (clientContext.notes)
        contextInfo += `- Notes: ${clientContext.notes}\n`;
      if (clientContext.client_since)
        contextInfo += `- Client Since: ${clientContext.client_since}\n`;
...
${
  clientContext
    ? `- ALWAYS reference ${clientContext.full_name} by name in your responses
...
${
  stylistContext
    ? `- Prioritize recommendations using ${stylistContext.color_line || 'their preferred color line'}
...
**Always end formulas with:** "⚠️ These recommendations are guidance for licensed professionals. Perform strand tests and verify all products are compatible with client's known sensitivities."`;
    ...
    const selectOptimalModel = (query: string): string => {
      ...
      if (
        query.length < 50 ||
        lowerQuery.match(/^(hi|hello|thanks|thank you|yes|no|okay|ok)$/i)
      ) {
      ...
      if (
        lowerQuery.includes('correction') ||
        lowerQuery.includes('fix') ||
        lowerQuery.includes('problem') ||
        lowerQuery.includes('damaged')
      ) {
    ...
    console.log(
      `AI response received in ${responseTime}ms using ${selectedModel}`
    );
