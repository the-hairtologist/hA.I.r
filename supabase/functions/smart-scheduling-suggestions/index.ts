...
serve(async (req) => {
  ...
  try {
    const { stylistId, clientId, timeRange } = await req.json();
...
[
  {
    ...
    if (LOVABLE_API_KEY) {
      const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            { role: 'user', content: 'Generate smart scheduling suggestions.' }
          ],
          max_tokens: 400,
        }),
      });
      ...
      if (response.ok) {
        ...
        try {
          ...
          return new Response(
            JSON.stringify({ suggestions, patterns: { dayCount, hourCount, serviceCount } }),
    ...
    const popularDay = Object.entries(dayCount).sort((a, b) => (b[1] as number) - (a[1] as number))[0]?.[0];
    const popularHour = Object.entries(hourCount).sort((a, b) => (b[1] as number) - (a[1] as number))[0]?.[0];
