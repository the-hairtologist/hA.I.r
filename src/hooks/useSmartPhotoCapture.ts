...
export const useSmartPhotoCapture = () => {
  ...
  const smartCapture = useCallback(async (
    ...
    try {
      ...
      const optimizedBlob = await imageCompression(new File([blob], 'photo.jpg'), {
        maxSizeMB: 1,
      ...
      const thumbnailBlob = await imageCompression(new File([blob], 'thumbnail.jpg'), {
        maxSizeMB: 0.1,
  ...
  const inferServiceType = (analysis: any): string => {
    ...
    if (analysis.color_fade_percentage && analysis.color_fade_percentage > 40) return 'Color Refresh';
