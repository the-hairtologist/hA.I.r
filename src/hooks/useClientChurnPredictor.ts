...
export const useClientChurnPredictor = (stylistId?: string) => {
  ...
  const predictChurn = async (client: any): Promise<ChurnPrediction | null> => {
    ...
    const canceledAppts = appointments.filter((a: any) => a.status === 'cancelled');
    ...
    const recentAppts = completedAppts.filter((a: any) => 
    ...
    const frequencyDecline = Math.max(0, (olderFrequency - recentFrequency) / Math.max(olderFrequency, 1));
    ...
    const uniqueServices = new Set(appointments.map((a: any) => a.service_type)).size;
    ...
    const riskLevel: 'critical' | 'high' | 'medium' | 'low' = 
      churnScore >= 0.7 ? 'critical' :
      churnScore >= 0.5 ? 'high' :
      churnScore >= 0.3 ? 'medium' : 'low';
  ...
  const generateActions = (churnScore: number, factors: ChurnFactor[], visitCount: number): ActionItem[] => {
  ...
  const savePredictionsToDatabase = async (stylistId: string, predictions: ChurnPrediction[]) => {
