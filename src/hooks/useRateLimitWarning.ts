 ...
 */
export function useRateLimitWarning(config: RateLimitConfig = {}): {
  ...
  const updateWarning = (
    ...
    const callsRemaining = Math.max(
      0,
      config.maxCallsPerMinute - callsLastMinute
    );
    ...
    if (callsLastMinute >= config.maxCallsPerMinute) {
      level = 'exceeded';
      message =
        'Rate limit reached. Please wait before making more AI requests.';
  ...
  const recordCall = useCallback(() => {
    ...
    log.info(
      'AI call recorded for rate limit tracking',
      'useRateLimitWarning',
      {
