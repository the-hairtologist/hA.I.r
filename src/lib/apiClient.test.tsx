...
describe('apiClient', () => {
  ...
  describe('withQueryRetry', () => {
    it('should execute query successfully', async () => {
      const mockQuery = vi
        .fn()
        .mockResolvedValue({ data: 'test', error: null });
    ...
    it('should handle errors from query', async () => {
      ...
      const result = (await withQueryRetry(mockQuery)) as {
        data: null;
        error: Error;
      };
  ...
  describe('apiClient.query', () => {
    ...
    it('should throw error when query fails', async () => {
      ...
      await expect(apiClient.query(mockQueryFn)).rejects.toThrow(
        'Database error'
      );
    });
  });
  ...
  describe('apiClient.parallel', () => {
    it('should execute multiple queries in parallel', async () => {
      ...
      const query2 = vi
        .fn()
        .mockResolvedValue({ data: 'result2', error: null });
