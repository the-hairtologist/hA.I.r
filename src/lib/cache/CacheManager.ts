...
class CacheManager {
   ...
   */
  invalidateRelated(types: Array<keyof typeof CACHE_STRATEGIES>, identifier?: string): void {
  ...
  invalidateAfterMutation(mutationType: 'appointment' | 'client' | 'payment' | 'service' | 'message' | 'formula', stylistId: string): void {
    ...
    switch (mutationType) {
      case 'appointment':
        this.invalidateRelated(['appointments', 'upcomingAppointments', 'analytics'], stylistId);
        break;
      case 'client':
        this.invalidateRelated(['clients', 'clientDetails', 'analytics'], stylistId);
        break;
      case 'payment':
        this.invalidateRelated(['payments', 'commissions', 'analytics'], stylistId);
        break;
      case 'service':
  ...
  getHealthMetrics() {
    ...
    return {
      ...
      health: stats.size < 100 ? 'healthy' : stats.size < 500 ? 'moderate' : 'high',
    };
  }
   ...
   */
  async preloadCritical(stylistId: string, dataTypes: Array<keyof typeof CACHE_STRATEGIES> = []): Promise<void> {
    logger.info('Preloading critical cache', 'CacheManager', { stylistId, types: dataTypes });
