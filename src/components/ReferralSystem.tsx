...
export const ReferralSystem = () => {
  ...
  const getRewardProgress = () => {
    ...
    if (count >= 10) return { tier: "Gold", next: "Max", progress: 100, reward: "6 months free" };
    if (count >= 5) return { tier: "Silver", next: "Gold (10)", progress: (count / 10) * 100, reward: "3 months free" };
    if (count >= 3) return { tier: "Bronze", next: "Silver (5)", progress: (count / 5) * 100, reward: "2 months free" };
    return { tier: "None", next: "Bronze (3)", progress: (count / 3) * 100, reward: "1 month free" };
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="brutal-border border-primary/20">
        <CardHeader>
          ...
          <CardDescription>
            Invite other stylists and earn free months when they join and stay active
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-muted-foreground">{referralData?.successful_referrals || 0} referrals</span>
            </div>
            ...
            <p className="text-sm text-muted-foreground">
              Next reward: <span className="font-semibold text-primary">{reward.reward}</span>
            </p>
          </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {[
              { referrals: "3 referrals", tier: "Bronze", reward: "1 month free" },
              { referrals: "5 referrals", tier: "Silver", reward: "2 months free" },
              { referrals: "10 referrals", tier: "Gold", reward: "3 months free" },
            ].map((item, i) => (
              <div
                key={i}
                className={cn(
                  "flex justify-between items-center p-3 rounded-lg border",
                  (referralData?.successful_referrals || 0) >= parseInt(item.referrals) &&
                    "bg-primary/5 border-primary/20"
                )}
              >
                <div>
                  <div className="font-medium">{item.tier}</div>
                  <div className="text-sm text-muted-foreground">{item.referrals}</div>
                </div>
