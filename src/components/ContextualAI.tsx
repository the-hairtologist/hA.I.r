...
export const ContextualAI = ({
  ...
  const [suggestion, setSuggestion] = useState<ContextualAISuggestion | null>(
    null
  );
        ...
        case 'formula':
          if (data?.clientId) {
            return {
              id: 'formula-history',
              message:
                "Looks like you're creating a formula—want me to pull your last mix for this client?",
        ...
        case 'appointment':
          if (data?.clientId && data?.lastAppointmentDate) {
            const daysSince = Math.floor(
              (Date.now() - new Date(data.lastAppointmentDate).getTime()) /
                (1000 * 60 * 60 * 24)
            );
            if (daysSince > 30) {
              return {
        ...
        case 'client':
          if (data?.recentFormulas?.length > 5) {
            return {
              ...
              message:
                'You have lots of formulas for this client. Want me to organize them by date or service type?',
        ...
        case 'schedule':
          if (data?.availableSlots && data.availableSlots.length < 3) {
            return {
              ...
              message:
                "You're running low on available slots this week. Want me to suggest optimal times to add?",
