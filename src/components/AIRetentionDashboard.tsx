...
export const AIRetentionDashboard = () => {
  ...
  const loadStylistId = async () => {
    ...
    if (result.error) {
      logger.error('Error loading stylist ID', result.error, {
  ...
  const loadRetentionData = async () => {
    ...
    try {
      ...
      if (scores.length > 0) {
        const aiInsights =
          await clientRetentionAI.getAIRetentionInsights(scores);
  ...
  const sendRetentionMessages = async () => {
    ...
    try {
      const sent = await clientRetentionAI.sendRetentionMessages(
        stylistId,
        riskScores
      );
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            ...
            <Button
              onClick={sendRetentionMessages}
              disabled={loading || atRiskCount === 0}
              size="sm"
