...
import {
  ...
  formatResetTime,
  getRateLimitColor,
} from '@/hooks/useRateLimitWarning';
...
export function RateLimitWarning() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 max-w-sm animate-in slide-in-from-bottom duration-300">
      <Alert
        className={`${isExceeded || isCritical ? 'bg-destructive/10 border-destructive' : 'bg-warning/10 border-warning'}`}
      >
        <AlertTriangle
          className={`h-4 w-4 ${getRateLimitColor(warning.level)}`}
        />
        <AlertDescription>
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span>
                  {warning.callsRemaining} request
                  {warning.callsRemaining !== 1 ? 's' : ''} left
                </span>
                {warning.resetIn > 0 && (
