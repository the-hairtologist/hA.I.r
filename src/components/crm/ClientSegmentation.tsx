...
import { Users, Star, AlertTriangle, UserPlus, Mail, Calendar } from "lucide-react";
    ...
    <div className="space-y-6">
      <Card className="brutal-border">
        <CardHeader>
          ...
          <CardDescription>
            Organize clients by behavior and value to target campaigns effectively
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              return (
                ...
                >
                  <CardContent className="p-4">
                    <div className={`${segment.bgColor} rounded-lg p-3 mb-3 w-fit`}>
        ...
        <Card className="brutal-border animate-fade-in">
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {segments.find(s => s.id === selectedSegment)?.actions.map((action, idx) => (
                <Button key={idx} variant="outline" size="sm" className="gap-2">
                  {action.toLowerCase().includes("send") ? <Mail className="h-4 w-4" /> : <Calendar className="h-4 w-4" />}
                  {action}
                </Button>
              ))}
            </div>
