...
import { TrendingUp, Calendar, DollarSign, Clock, Sparkles } from "lucide-react";
...
export const WeeklySummaryCard = ({ stylistId, className }: WeeklySummaryCardProps) => {
  ...
  const loadWeeklySummary = async () => {
    try {
      ...
      const earnings = thisWeek?.reduce((sum, apt) => sum + (apt.stylist_services?.price || 0), 0) || 0;
      ...
      const growthPercent = prevWeek && prevWeek.length > 0
        ? Math.round(((appointments - prevWeek.length) / prevWeek.length) * 100)
        : 0;
  ...
  if (loading) {
    return (
      <Card className={cn("border-[3px] border-foreground shadow-[4px_4px_0px_0px_hsl(var(--foreground))] animate-pulse", className)}>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className={cn(
              ...
              summary.growthPercent > 0 ? "text-success" : summary.growthPercent < 0 ? "text-destructive" : "text-foreground"
            )}>
