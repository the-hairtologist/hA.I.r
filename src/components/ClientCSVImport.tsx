    ...
    try {
      ...
      for (const client of clients) {
        try {
          ...
          if (error) {
            errors.push(
              `Failed to import ${client.full_name}: ${error.message}`
            );
          } else {
            successCount++;
          }
      ...
      <CardContent className="space-y-4">
        ...
        <Alert>
          ...
          <AlertDescription className="text-xs">
            <strong>CSV Format:</strong> Include columns: full_name (required),
            email, phone, hair_type, allergies, notes
          </AlertDescription>
        </Alert>
      </CardContent>
    </Card>
