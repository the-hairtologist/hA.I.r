...
export function StylistPerformance() {
  ...
  const { data: performance, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const revenue = recentAppointments?.reduce((sum, apt) => 
        sum + Number(apt.stylist_services?.price || 0), 0) || 0;
            ...
            <div className="flex items-center justify-between p-4 rounded-lg border-2 border-border">
              <div>
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  ...
                  <span className="font-semibold text-foreground">${performance.revenue.toFixed(2)}</span>
                  <Badge variant="secondary">⭐ {performance.rating} ({performance.totalReviews} reviews)</Badge>
                </div>
              </div>
            </div>
          </div>
