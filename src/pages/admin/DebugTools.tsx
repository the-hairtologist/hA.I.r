...
export default function DebugTools() {
  ...
  const [testResults, setTestResults] = useState<
    Array<{ test: string; status: 'pass' | 'fail'; message: string }>
  >([]);
   ...
   */
  const triggerError = (
    type: 'unhandled' | 'promise' | 'network' | 'boundary'
  ) => {
    try {
      switch (type) {
        case 'unhandled':
        ...
        case 'promise':
          ...
          break;
        case 'network':
          fetch('https://invalid-domain-that-does-not-exist.com').catch(err =>
            logger.error('Test network error', err)
          );
          break;
        case 'boundary':
      ...
      <Tabs defaultValue="overview" className="space-y-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span>Database</span>
                  <Badge
                    variant={healthData?.database ? 'default' : 'destructive'}
                  >
            ...
            <Card>
              ...
              <CardContent className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span>
                    {(journeySummary.sessionDuration / 1000 / 60).toFixed(1)}{' '}
                    min
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <Badge
                    variant={
                      journeySummary.errorCount > 0 ? 'destructive' : 'default'
                    }
                  >
        ...
        <TabsContent value="errors" className="space-y-4">
          <Card>
            ...
            <CardContent className="grid gap-4 grid-cols-1 sm:grid-cols-2">
              <Button
                onClick={() => triggerError('unhandled')}
                variant="outline"
        ...
        <TabsContent value="logs" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Recent log entries from the application
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="journey" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>
                User Journey ({journeyEvents.length} events)
              </CardTitle>
              <CardDescription>
                ...
                {journeyEvents.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {journeyEvents.map((event, idx) => (
                      <div
                        key={idx}
                        className="text-xs border-l-2 border-primary/20 pl-3 py-1"
                      >
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-muted-foreground">
                            ...
                            {(
                              (event.timestamp -
                                journeySummary.sessionDuration) /
                              1000
                            ).toFixed(1)}
                            s
                          </span>
