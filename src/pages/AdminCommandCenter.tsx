    ...
    try {
      ...
      const [
        ...
        supabase
          .from('appointments')
          .select(
            '*, stylist:stylist_profiles(user:profiles(full_name)), client:client_profiles(user:profiles(full_name))'
          )
          .order('created_at', { ascending: false })
          .limit(10),
        supabase
      ...
      const completionRate = appointmentCountResult.count
        ? (
            ((completedApptResult.count || 0) / appointmentCountResult.count) *
            100
          ).toFixed(1)
        : 0;
      ...
      setBusinessMetrics({
        totalRevenue,
        avgRevenue: appointmentCountResult.count
          ? (totalRevenue / appointmentCountResult.count).toFixed(2)
          : 0,
          ...
          <Card className="border-4 border-foreground shadow-brutal">
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                ...
                <Activity
                  className={`h-8 w-8 ${stats.recentErrors === 0 ? 'text-success' : 'text-warning'}`}
                />
              </div>
            </CardContent>
          </Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {recentUsers.map(user => (
                    <div
                      key={user.id}
                      className="flex flex-col md:flex-row md:items-center justify-between p-4 border-2 border-foreground rounded-lg gap-2"
                    >
                  ...
                  {recentAppointments.map(appt => (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {appt.stylist?.user?.full_name} →{' '}
                          {appt.client?.user?.full_name}
                        </p>
                        <p className="text-xs text-muted-foreground">
                      ...
                      <Badge
                        variant={
                          appt.status === 'completed' ? 'default' : 'secondary'
                        }
                      >
                        {appt.status}
                      </Badge>
                ...
                <div className="space-y-3">
                  {errorLogs.length > 0 ? (
                    errorLogs.map(error => (
                      <div
                        key={error.id}
                        className="p-3 border-2 border-destructive/20 bg-destructive/5 rounded-lg"
                      >
