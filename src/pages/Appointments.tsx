...
const Appointments = () => {
  ...
  const { handleSubmit: handleBulkComplete, isSubmitting: bulkUpdating } =
    useFormSubmit(
      async () => {
  ...
  const {
    ...
    refetch: refetchStylistAppointments,
  } = useAppointmentsByStylist(
    profilesLoaded && userRole === 'stylist' ? stylistProfile?.id : null
  );
  const stylistAppointments = stylistAppointmentsData?.appointments || [];

  const {
    data: clientAppointments = [],
    isLoading: loadingClientAppointments,
    refetch: refetchClientAppointments,
  } = useAppointmentsByClient(
    profilesLoaded && userRole === 'client' ? clientProfile?.id : null
  );
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (newStatus === 'completed') {
            showCelebration(
              'income-secured',
              `${clientName} - appointment completed!`
            );
  ...
  const todayAppointments = appointments
    .filter((apt: any) => {
      ...
      return (
        format(aptDate, 'yyyy-MM-dd') === format(today, 'yyyy-MM-dd') &&
  ...
  return (
    ...
    >
      ...
      >
        <Tabs
          value={viewMode}
          onValueChange={v => setViewMode(v as 'list' | 'calendar' | 'week')}
          className="space-y-6"
        >
              ...
              <ContextualAI
                ...
                onAction={action => {
                  if (action === 'send-rebook-reminder') {
                    toast.info(
                      'Rebook reminders are being configured for this feature.',
                      {
                        description:
                          'Automated reminders will be available soon',
                      }
                    );
                  }
                }}
              ...
              <div className="mb-6">
                <SmartSchedulingSuggestions
                  ...
                  onSelectTime={datetime => {
                    toast.success(
                      'Time selected! You can now create an appointment for this time.'
                    );
                              ...
                              <Button
                                ...
                                onClick={async e => {
                                  ...
                                  try {
                                    ...
                                    if (
                                      existingTimer &&
                                      !existingTimer.end_time
                                    ) {
                                      toast.info(
                                        'Timer already running for this appointment'
                                      );
                                      return;
                                    }
                                ...
                                clientName={
                                  userRole === 'client'
                                    ? apt.stylist?.business_name ||
                                      apt.stylist?.user?.full_name ||
                                      'Stylist'
                                    : apt.client?.user?.full_name || 'Client'
                                }
                                serviceType={apt.service_type}
                                stylistId={apt.stylist_id}
                ...
                <div className="pt-4 border-t space-y-2">
                  ...
                  <div className="grid gap-2">
                    <Button
                      ...
                      onClick={() => {
                        navigate(
                          `/clients?view=${selectedAppointment.client_id}`
                        );
                        setDetailsOpen(false);
                      }}
                    >
                    ...
                    <Button
                      ...
                      onClick={() => {
                        navigate(
                          `/formulas?client=${selectedAppointment.client_id}`
                        );
                        setDetailsOpen(false);
                      }}
                    >
                    ...
                    <Button
                      ...
                      onClick={() => {
                        navigate(
                          `/formulas?new=true&client=${selectedAppointment.client_id}`
                        );
                        setDetailsOpen(false);
                      }}
                    >
      ...
      <ConfirmDialog
        ...
        confirmText="Confirm"
        variant={
          confirmDialog.title.toLowerCase().includes('cancel')
            ? 'destructive'
            : 'default'
        }
      />
