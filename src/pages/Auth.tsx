...
const Auth = () => {
  ...
  const handlePasswordReset = createSafeHandler(async () => {
    if (!state.resetEmail) {
      dispatch({
        type: 'SET_ERRORS',
        errors: { resetEmail: 'Email is required for password reset' },
      });
      return;
    }
  ...
  const handleSocialSignIn = createSafeHandler(
    async (provider: 'google' | 'apple' | 'azure') => {
      // Track social sign-in attempt
      await eventTracker.track({
        eventName: 'social_signin_started',
        eventCategory: 'auth',
        eventData: { provider },
      });
      ...
      if (error) {
        // Track failure
        await eventTracker.track({
          eventName: 'social_signin_failed',
          eventCategory: 'auth',
          eventData: { provider, reason: error.message },
        });
        throw error;
      }

      // Success tracking will happen in onAuthStateChange
      log.info(`${provider} OAuth initiated`, 'Auth', { provider });
    },
    'Social Sign In'
  );
  ...
  return (
    <div
      className="min-h-screen flex items-center justify-center bg-primary p-4 relative overflow-hidden"
      style={{
        backgroundImage: `
      ...
      >
        <Card className="w-full border-[3px] sm:border-[4px] border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] sm:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-background max-w-md mx-auto rounded-none">
          <CardHeader className="text-center space-y-2 sm:space-y-3 px-4 sm:px-6 pt-5 sm:pt-6 border-b-[3px] border-black">
            ...
            <CardDescription className="text-xs sm:text-sm md:text-base font-pixel text-foreground/80 text-center px-2 uppercase">
              {state.isRecoveryMode
                ? 'Create New Password'
                : 'For Professional Hair Stylists'}
            </CardDescription>
          </CardHeader>
          <CardContent className="px-4 sm:px-6 pb-5 sm:pb-6 pt-5 sm:pt-6">
            {state.isRecoveryMode ? (
              <form onSubmit={handleUpdatePassword} className="space-y-4">
                <div className="space-y-2">
                  ...
                  {state.errors.newPassword && (
                    <FormFieldError message={state.errors.newPassword} />
                  )}
                </div>
                <div className="space-y-2">
                  <Label
                  ...
                  {state.errors.confirmPassword && (
                    <FormFieldError message={state.errors.confirmPassword} />
                  )}
                </div>
                <Button
                  type="submit"
                  ...
                  <form onSubmit={handleSignIn} className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        ...
                        <span className="font-pixel text-xs sm:text-sm uppercase">
                          Sign in with Google
                        </span>
                      </Button>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      />
                      {state.errors.email && (
                        <FormFieldError message={state.errors.email} />
                      )}
                    </div>
                    <div className="space-y-2">
                      <Label
                      ...
                      {state.errors.password && (
                        <FormFieldError message={state.errors.password} />
                      )}
                    </div>
                    <Button
                      type="submit"
                  ...
                  <form onSubmit={handleSignUp} className="space-y-4">
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-2 sm:gap-3">
                        ...
                        >
                          ...
                          <span className="text-xs sm:text-sm font-pixel">
                            Google
                          </span>
                        </Button>
                    ...
                    <div className="space-y-2">
                      ...
                      {state.errors.fullName && (
                        <FormFieldError message={state.errors.fullName} />
                      )}
                    </div>
                    <div className="space-y-2">
                      <Label
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                {state.errors.resetEmail && (
                  <FormFieldError message={state.errors.resetEmail} />
                )}
              </div>
              <Button
