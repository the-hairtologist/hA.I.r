...
test.describe('Client Mobile Workflow - All Devices', () => {
  for (const { name, device } of MOBILE_DEVICES) {
    test(`Client: View appointments on ${name}`, async ({ page }) => {
      ...
      const hasAppointments = content.includes('appointment') || content.includes('schedule');
...
test.describe('Mobile Performance & Accessibility', () => {
  ...
  test('should handle slow 3G connection gracefully', async ({ page, context }) => {
    // Simulate slow 3G
...
test.describe('Mobile Navigation Tests', () => {
  ...
  test('should maintain state during navigation on mobile', async ({ page }) => {
    test.use(devices['iPhone 15 Pro']);
