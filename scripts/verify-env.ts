...
requiredSecrets.forEach(({ name, description, pattern }) => {
  ...
  if (!value) {
    ...
    console.log(
      `   ${colors.red}⚠️  App will fail without this!${colors.reset}\n`
    );
    hasErrors = true;
  } else if (!pattern.test(value)) {
    console.log(`${colors.yellow}⚠️  INVALID FORMAT: ${name}${colors.reset}`);
    console.log(
      `   ${colors.yellow}↳ Value doesn't match expected pattern${colors.reset}`
    );
    console.log(
      `   ${colors.yellow}↳ Current: ${value.substring(0, 15)}...${colors.reset}\n`
    );
    hasWarnings = true;
  } else {
    const maskedValue =
...
optionalSecrets.forEach(({ name, description, pattern }) => {
  ...
  if (!value) {
    ...
    console.log(
      `   ${colors.yellow}↳ ${description} (optional but recommended)${colors.reset}\n`
    );
    hasWarnings = true;
  } else if (!pattern.test(value)) {
    console.log(`${colors.yellow}⚠️  INVALID FORMAT: ${name}${colors.reset}`);
...
supabaseVars.forEach(name => {
  ...
  if (value) {
    ...
    console.log(
      `${colors.yellow}⚠️  ${name}: Not detected (should be auto-configured)${colors.reset}`
    );
  }
});
...
if (hasErrors) {
  ...
  console.log(
    `${colors.red}Missing required secrets. Add them in Lovable Cloud:${colors.reset}`
  );
  ...
  console.log(
    `${colors.red}4. Add the missing secrets listed above${colors.reset}\n`
  );
  process.exit(1);
} else if (hasWarnings) {
  console.log(
  ...
  console.log(
    `${colors.yellow}Some optional secrets are missing or invalid.${colors.reset}`
  );
  console.log(
    `${colors.yellow}The app will work, but some features may be limited.${colors.reset}\n`
  );
  process.exit(0);
} else {
  console.log(
  ...
  console.log(
    `${colors.green}All required secrets are configured correctly!${colors.reset}`
  );
  console.log(
    `${colors.green}Your app is ready for deployment. 🚀${colors.reset}\n`
  );
  process.exit(0);
}
